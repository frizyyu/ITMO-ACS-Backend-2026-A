...
class VacancyController extends BaseController {
    private async assertExternalReferences(
        ...
        await serviceGet(
            SETTINGS.COMPANY_SERVICE_URL,
            `/internal/v1/employer-profiles/${employerProfileId}`,
        );
        await serviceGet(
            SETTINGS.DICTIONARY_SERVICE_URL,
            `/internal/v1/industries/${industryId}`,
        );
        await serviceGet(
            SETTINGS.DICTIONARY_SERVICE_URL,
            `/internal/v1/experience-levels/${experienceLevelId}`,
        );
    }
    ...
    private async assertEmployerOwnership(
        ...
        const result = await serviceGet<{ owned: boolean }>(
            SETTINGS.COMPANY_SERVICE_URL,
            `/internal/v1/employer-profiles/${employerProfileId}/ownership?user_id=${request.user.id}&company_id=${companyId}`,
        );
